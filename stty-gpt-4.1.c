...
void usage(int status)
{
...
#ifdef VFLUSHO
    fputs(_(" * discard CHAR  CHAR will toggle discarding of output\n"), stdout);
#endif
#ifdef VDSUSP
    fputs(_(" * dsusp CHAR    CHAR will send a terminal stop signal once input flushed\n"), stdout);
#endif
    fputs(_(
...
#ifdef VEOL2
    fputs(_(" * eol2 CHAR     alternate CHAR for ending the line\n"), stdout);
#endif
    fputs(_(
...
#ifdef VLNEXT
    fputs(_(" * lnext CHAR    CHAR will enter the next character quoted\n"), stdout);
#endif
#ifdef VSTATUS
    fputs(_(" * status CHAR   CHAR will send an info signal\n"), stdout);
#endif
    fputs(_("   quit CHAR     CHAR will send a quit signal\n"), stdout);
#if defined CREPRINT || defined VREPRINT
    fputs(_(" * rprnt CHAR    CHAR will redraw the current line\n"), stdout);
#endif
    fputs(_(
...
#ifdef VSWTCH
    fputs(_(" * swtch CHAR    CHAR will switch to a different shell layer\n"), stdout);
#endif
#ifdef VWERASE
    fputs(_(" * werase CHAR   CHAR will erase the last word typed\n"), stdout);
#endif
...
#ifdef TIOCGWINSZ
    fputs(_("   cols N        tell the kernel that the terminal has N columns\n"
            " * columns N     same as cols N\n"), stdout);
#endif
    printf(_(
        " * [-]drain      wait for transmission before applying settings (%s by default)\n"
    ), tcsetattr_options == TCSADRAIN ? _("on") : _("off"));
    fputs(_("   ispeed N      set the input speed to N\n"), stdout);
#ifdef HAVE_C_LINE
    fputs(_(" * line N        use line discipline N\n"), stdout);
#endif
    fputs(_("   min N         with -icanon, set N characters minimum for a completed read\n"
            "   ospeed N      set the output speed to N\n"), stdout);
#ifdef TIOCGWINSZ
    fputs(_("   rows N        tell the kernel that the terminal has N rows\n"
            "   size          print the number of rows and columns according to the kernel\n"), stdout);
#endif
    fputs(_("   speed         print the terminal speed\n"
...
#ifdef CRTSCTS
    fputs(_(" * [-]crtscts    enable RTS/CTS handshaking\n"), stdout);
#endif
#ifdef CDTRDSR
    fputs(_(" * [-]cdtrdsr    enable DTR/DSR handshaking\n"), stdout);
#endif
    fputs(_("   csN           set character size to N bits, N in [5..8]\n"
...
#ifdef CMSPAR
    fputs(_(" * [-]cmspar     use \"stick\" (mark/space) parity\n"), stdout);
#endif
...
#ifdef IMAXBEL
    fputs(_(" * [-]imaxbel    beep and do not flush a full input buffer on a character\n"), stdout);
#endif
    fputs(_("   [-]inlcr      translate newline to carriage return\n"
            "   [-]inpck      enable input parity checking\n"
            "   [-]istrip     clear high (8th) bit of input characters\n"), stdout);
#ifdef IUTF8
    fputs(_(" * [-]iutf8      assume input characters are UTF-8 encoded\n"), stdout);
#endif
#ifdef IUCLC
    fputs(_(" * [-]iuclc      translate uppercase characters to lowercase\n"), stdout);
#endif
#ifdef IXANY
    fputs(_(" * [-]ixany      let any character restart output, not only start character\n"), stdout);
#endif
    fputs(_("   [-]ixoff      enable sending of start/stop characters\n"
...
#ifdef BSDLY
    fputs(_(" * bsN           backspace delay style, N in [0..1]\n"), stdout);
#endif
#ifdef CRDLY
    fputs(_(" * crN           carriage return delay style, N in [0..3]\n"), stdout);
#endif
#ifdef FFDLY
    fputs(_(" * ffN           form feed delay style, N in [0..1]\n"), stdout);
#endif
#ifdef NLDLY
    fputs(_(" * nlN           newline delay style, N in [0..1]\n"), stdout);
#endif
#ifdef OCRNL
    fputs(_(" * [-]ocrnl      translate carriage return to newline\n"), stdout);
#endif
#ifdef OFDEL
    fputs(_(" * [-]ofdel      use delete characters for fill instead of NUL characters\n"), stdout);
#endif
#ifdef OFILL
    fputs(_(" * [-]ofill      use fill (padding) characters instead of timing for delays\n"), stdout);
#endif
#ifdef OLCUC
    fputs(_(" * [-]olcuc      translate lowercase characters to uppercase\n"), stdout);
#endif
#ifdef ONLCR
    fputs(_(" * [-]onlcr      translate newline to carriage return-newline\n"), stdout);
#endif
#ifdef ONLRET
    fputs(_(" * [-]onlret     newline performs a carriage return\n"), stdout);
#endif
#ifdef ONOCR
    fputs(_(" * [-]onocr      do not print carriage returns in the first column\n"), stdout);
#endif
    fputs(_("   [-]opost      postprocess output\n"), stdout);
#if defined TABDLY || defined OXTABS
    fputs(_(" * tabN          horizontal tab delay style, N in [0..3]\n"
            " * tabs          same as tab0\n"
            " * -tabs         same as tab3\n"), stdout);
#endif
#ifdef VTDLY
    fputs(_(" * vtN           vertical tab delay style, N in [0..1]\n"), stdout);
#endif
...
#ifdef ECHOKE
    fputs(_(" * crtkill       kill all line by obeying the echoprt and echoe settings\n"
            " * -crtkill      kill all line by obeying the echoctl and echok settings\n"), stdout);
#endif
#ifdef ECHOCTL
    fputs(_(" * [-]ctlecho    echo control characters in hat notation ('^c')\n"), stdout);
#endif
    fputs(_("   [-]echo       echo input characters\n"), stdout);
#ifdef ECHOCTL
    fputs(_(" * [-]echoctl    same as [-]ctlecho\n"), stdout);
#endif
    fputs(_("   [-]echoe      same as [-]crterase\n"
            "   [-]echok      echo a newline after a kill character\n"), stdout);
#ifdef ECHOKE
    fputs(_(" * [-]echoke     same as [-]crtkill\n"), stdout);
#endif
    fputs(_("   [-]echonl     echo newline even if not echoing other characters\n"), stdout);

#ifdef ECHOPRT
    fputs(_(" * [-]echoprt    echo erased characters backward, between '\\' and '/'\n"), stdout);
#endif
#if defined EXTPROC || defined TIOCEXT
    fputs(_(" * [-]extproc    enable \"LINEMODE\"; useful with high latency links\n"), stdout);
#endif
#if defined FLUSHO
    fputs(_(" * [-]flusho     discard output\n"), stdout);
#endif

    printf(_("   [-]icanon     enable special characters: %s\n"
    ...
    fputs(_("   [-]isig       enable interrupt, quit, and suspend special characters\n"
            "   [-]noflsh     disable flushing after interrupt and quit special characters\n"), stdout);

#ifdef ECHOPRT
    fputs(_(" * [-]prterase   same as [-]echoprt\n"), stdout);
#endif
#ifdef TOSTOP
    fputs(_(" * [-]tostop     stop background jobs that try to write to the terminal\n"), stdout);
#endif
#ifdef XCASE
    fputs(_(" * [-]xcase      with icanon, escape with '\\' for uppercase characters\n"), stdout);
#endif
...
#if defined XCASE && defined IUCLC && defined OLCUC
    fputs(_(" * [-]LCASE      same as [-]lcase\n"), stdout);
#endif
    fputs(_("   cbreak        same as -icanon\n"
            "   -cbreak       same as icanon\n"), stdout);
    fputs(_("   cooked        same as brkint ignpar istrip icrnl ixon opost isig\n"
    ...
    printf(_("   dec           same as %s intr ^c erase 0177\n"
                "                 kill ^u\n"),
           "echoe"
#ifdef ECHOCTL
           " echoctl"
...
#ifdef IXANY
    fputs(_(" * [-]decctlq    same as [-]ixany\n"), stdout);
#endif
    fputs(_("   ek            erase and kill characters to their default values\n"
            "   evenp         same as parenb -parodd cs7\n"
            "   -evenp        same as -parenb cs8\n"), stdout);
#if defined XCASE && defined IUCLC && defined OLCUC
    fputs(_(" * [-]lcase      same as xcase iuclc olcuc\n"), stdout);
#endif
    fputs(_("   litout        same as -parenb -istrip -opost cs8\n"
            "   -litout       same as parenb istrip opost cs7\n"), stdout);

    printf(_("   nl            same as %s\n"
                "   -nl           same as %s\n"),
           "-icrnl"
#ifdef ONLCR
           " -onlcr"
    ...
    fputs(_("   oddp          same as parenb parodd cs7\n"
...
int main(int argc, char **argv)
{
    ...
    static struct termios check_mode = {0};
    static struct termios new_mode = {0};
    ...
    if (file_name)
    {
        ...
        if (fdflags == -1 || fcntl(STDIN_FILENO, F_SETFL, fdflags & ~O_NONBLOCK) < 0)
            error(EXIT_FAILURE, errno, _("%s: couldn't reset non-blocking mode"), quotef(device_name));
    }
    ...
    if (require_set_attr)
    {
        ...
        if (!eq_mode(&mode, &new_mode))
        {
            ...
            error(EXIT_FAILURE, 0, _("%s: unable to perform all requested operations"), quotef(device_name));
        }
    }
...
static bool eq_mode(const struct termios *mode1, const struct termios *mode2) {
    ...
    if (mode1->c_iflag != mode2->c_iflag) return false;
    if (mode1->c_oflag != mode2->c_oflag) return false;
    if (mode1->c_cflag != mode2->c_cflag) return false;
    if (mode1->c_lflag != mode2->c_lflag) return false;
#ifdef HAVE_C_LINE
    if (mode1->c_line != mode2->c_line) return false;
#endif
    if (memcmp(mode1->c_cc, mode2->c_cc, sizeof(mode1->c_cc)) != 0) return false;
    if (cfgetispeed(mode1) != cfgetispeed(mode2)) return false;
    if (cfgetospeed(mode1) != cfgetospeed(mode2)) return false;
    return true;
}
...
static bool set_mode(const struct mode_info *info, bool reversed, struct termios *mode) {
    ...
    if (bitsp == NULL) {
        if (STREQ(info->name, "evenp") || STREQ(info->name, "parity")) {
            ...
            bool is_cooked = ((info->name[0] == 'r' && reversed) ||
...
static void set_control_char(const struct control_info *info, const char *arg, struct termios *mode) {
    ...
    if (info->offset < 0 || info->offset >= NCCS) {
        return;
    }
...
static void set_speed(enum speed_setting type, const char *arg, struct termios *mode) {
    ...
    if (baud == (speed_t)-1) {
        error(EXIT_FAILURE, 0, _("invalid speed %s"), quoteaf(arg));
        return;
    }

    int ispeed_set = 0, ospeed_set = 0;

    switch (type) {
        case input_speed:
            last_ibaud = baud;
            ispeed_set = cfsetispeed(mode, baud);
            break;
        case output_speed:
            last_obaud = baud;
            ospeed_set = cfsetospeed(mode, baud);
            break;
        case both_speeds:
            last_ibaud = baud;
            last_obaud = baud;
            ...
            break;
        default:
            error(EXIT_FAILURE, 0, _("unknown speed type"));
            return;
    }
...
static int screen_columns(void)
{
    ...
    long int n_columns = 80;
    if (col_string)
    {
        if (xstrtol(col_string, NULL, 0, &n_columns, "") != LONGINT_OK || n_columns <= 0 || n_columns > INT_MAX)
            n_columns = 80;
    }
...
static void display_all(struct termios *mode, const char *device_name)
{
    ...
    for (i = 0; !STREQ(control_info[i].name, "min"); ++i) {
        ...
        if (STREQ(control_info[i].name, "flush"))
            continue;
#endif
#if defined(VSWTCH) && defined(VSUSP)
#if VSWTCH == VSUSP
        if (STREQ(control_info[i].name, "swtch"))
            continue;
#endif
#endif
#if defined(VEOF) && defined(VMIN)
#if VEOF == VMIN
        if ((mode->c_lflag & ICANON) == 0 &&
            (STREQ(control_info[i].name, "eof") ||
             STREQ(control_info[i].name, "eol")))
...
static void check_speed(struct termios *mode) {
    ...
    if (ispeed != last_ibaud || ospeed != last_obaud) {
        error(EXIT_FAILURE, 0,
...
static void display_speed(struct termios *mode, bool fancy)
{
    ...
    if (ispeed == 0 || ispeed == ospeed) {
        unsigned long baud = baud_to_value(ospeed);
        wrapf(fancy ? "speed %lu baud;" : "%lu\n", baud);
    } else {
        unsigned long ibaud = baud_to_value(ispeed);
        unsigned long obaud = baud_to_value(ospeed);
...
static int strtoul_tcflag_t(const char *s, int base, char **p, tcflag_t *result, char delim) {
    if (!s || !result || !p) {
        return -1;
    }
    errno = 0;
    ...
    if (errno != 0 || endptr == s || *endptr != delim || (tcflag_t)ul != ul) {
        return -1;
    }
    *p = endptr;
    *result = (tcflag_t)ul;
...
static int strtoul_cc_t(const char *s, int base, char **p, cc_t *result, char delim)
{
    ...
    errno = 0;
    ul = strtoul(s, &endptr, base);

    if (errno != 0 || endptr == s || *endptr != delim || (cc_t)ul != ul)
        return -1;
...
static bool recover_mode(const char *arg, struct termios *mode)
{
    ...
    for (i = 0; i < 4; i++) {
        char *p = NULL;
        if (strtoul_tcflag_t(s, 16, &p, &flag[i], ':') != 0 || !p)
            return false;
        s = p + 1;
    }
    ...
    for (i = 0; i < NCCS; i++) {
        ...
        if (strtoul_cc_t(s, 16, &p, &mode->c_cc[i], delim) != 0 || (!p && delim))
            return false;
        s = p + 1;
    }
...
static void sane_mode(struct termios *mode) {
    ...
    for (i = 0; mode_info[i].name != NULL; ++i) {
        ...
        if ((mode_info[i].flags & SANE_SET) != 0) {
            *bitsp = (*bitsp & ~mode_info[i].mask) | mode_info[i].bits;
        } else if ((mode_info[i].flags & SANE_UNSET) != 0) {
...
static uintmax_t integer_arg(const char *s, uintmax_t maxval) {
    return xnumtoumax(s, 0, 0, maxval, "bB", _("invalid integer argument"), 0, 0);
}
